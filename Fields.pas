...
procedure Expand(X,Y:Int8;i:Integer);
...
//С двумя дырками может фигура создаваться
//Например x.x.x -после обьединения у нас дырка закроется
//Но до Combine мы не можем закрыть дырку.
 // {$IFDEF LogError}
 // if (Figures[i].Hole) and (Figures[i2].Hole) then    //For Debug
//    Debug.Log('Combine Error: Cannot create figure with two holes');
//  {$ENDIF}
 ...
 // Figures[i].Length := Figures[i].Length +  Figures[i2].Length;    //Теперь неверно в общем случае - может быть перекрытие

 With Figures[i] do
   begin
   TurnMod := Turn mod 4 + 4;
   if XIndex[Turn]=0 then Revert := ((StartY-Figures[i2].StartY)/YIndex[TurnMod]>0.1)
                     else Revert := ((StartX-Figures[i2].StartX)/XIndex[TurnMod]>0.1);
   if Revert then
     begin
       StartX := Figures[i2].StartX;
       StartY := Figures[i2].StartY;
     end;
   l := 0;
   m := 0;
   Hole :=False;
   ...
   repeat
     inc(l);
     Px := StartX+l*XIndex[TurnMod];
     Py := StartY+l*YIndex[TurnMod];
     if  OutofBounds(Px,Py) then
       begin
         if l-m=1 then  //Возможно никогда не входит в эту ветку
           begin
             m:=0;
             dec(l);
           end;
         if m>0 then Hole := True;

         EndFigure := True;
       end
     else  if Data[Px,Py].Color<>Color then
       begin
         ...
         {$IFDEF LogError}
         if (abs(Data[Px,Py].Color-Color)=2) and (not EndFigure)  then    //For Debug
           Debug.Log('Combine Error: Cannot create splitted figure');
         {$ENDIF}
       end;
   until EndFigure;
   Length := l;
   if Hole then dec(Length);
             ...
             if (j2>=0) and (i<4) then  //xvx  //and (CenterHole(X,Y,i))) then // xvx
               begin
                 ...
                 if ((j>=0) and (j<>j2)) or ((j3>=0) and (j3<>j2)) then
                   begin
                     if ((j>=0) and (j<>j2)) then Combine(X,Y,j2,j)
               ...
               if j>=0 then
                 ...
                 if Data[X,Y].Figure[k]>=0 then  //С другой стороны проставили фигуру
                  //if Figures[Data[X,Y].Figure[k]].Hole=False then
              ...
              if Data[X,Y].Figure[k]>=0 then
               if Figures[Data[X,Y].Figure[k]].Hole=False  then
                Join(Data[X,Y].Figure[k],k,X+2*XIndex[i],Y+2*YIndex[i],2)
                 ...
                 begin
                   // Найти начало дыры и фигуру именно туда установить.
                   SetFigures(X,Y,i,2); //Установить фигуру
                   if i>=4 then Expand(X,Y,i);
                 end
              else
                begin
                  SetFigures(X,Y,i,2);
...
procedure TField.Expand;
var j,k : Integer;
begin
 j:= 1;
 k:= (i + 4) mod 8;
 while (not OutOfBounds(X+j*XIndex[k],Y+j*YIndex[k])) and
       (Data[X+j*XIndex[k],Y+j*YIndex[k]].Color=Data[X,Y].Color) do
   begin
     if Data[X+j*XIndex[k],Y+j*YIndex[k]].Figure[k]=-1 then break;
     // Расширяемся только в направлении уже известных фигур.

     if k<4 then
       begin
         Figures[High(Figures)].StartX :=X+j*XIndex[k];
         Figures[High(Figures)].StartY :=Y+j*YIndex[k];
       end;
     inc(Figures[High(Figures)].Length);
     Data[X+j*XIndex[k],Y+j*YIndex[k]].Figure[i] := High(Figures);
     inc(j);
   end;
end;
...
begin
  ...
  if Data[X+Holemul*XIndex[Turn],Y+Holemul*YIndex[Turn]].Figure[(Turn+4) mod 8] <0 then
...
begin
 // if Turn>7 then
   // Turn := 0;
  { if X+Offset*XIndex[Turn]>GridSize-1 then
   begin
     X:=0;
   end;
  if Y+Offset*YIndex[Turn]>GridSize-1 then
  begin
     X:=0;
     exit;
  end; }
...
begin
  GetEnd(EndX,EndY,i);
  {$IFDEF LogError}
  if OutOfBounds(EndX,EndY) then    //For Debug
    Debug.Log('SplitError: End of line iccorect');
  {$ENDIF}
  ...
  j := Data[Buf.StartX,Buf.StartY].Figure[Turn];

  if (Buf.Length>1) and ((j=-1) or (j=i)) then
    begin
      Figures := Figures+[buf];
      k := Length(Figures)-1;
      RefreshScore(i);
...
begin
  ...
  Y1 :=  Figures[i].StartY;
  Turn := Figures[i].Turn;
  GetFreePlace(X1,Y1,Turn,1,Hole);//Не расчитан на фигуры типа xxxxx.x . А надо?
  X1:=X1+(Hole+1)*XIndex[Turn];
  Y1:=Y1+(Hole+1)*YIndex[Turn];
  {$IFDEF LogError}
  if OutOfBounds(X1,Y1) then
    Debug.Log('CloseHole Error: Point '+IntToStr(X1)+':'+IntToStr(Y1)+' is out of field');
  if Data[X1,Y1].Color<>1 then
    Debug.Log('CloseHole Error: Point '+IntToStr(X1)+':'+IntToStr(Y1)+' is not void');
 {$ENDIF}
...
begin
  ...
  if Length(Field.Figures)=0 then Field.FindFreeTurn(TurnColor)
    ...
    begin
      ...
      Debug.Log('Ветвей: '+IntToStr(Field.Branches));
    end;
